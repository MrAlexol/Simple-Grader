...
async function callOpenAIForHint({ level, task, code }) {
  ...
  const policy =
    ...
    `- Не пиши полный код программы целиком.\n` +
    `- Пиши простым человеческим языком, без заумных формулировок.\n` +
    `- Убедись, что программа написана на Python 3. Если нет, укажи в своем ответе, что программа должна быть написана на Python.\n` +
    `- Если видишь 1-2 синтаксические ошибки, явно укажи их место. Если таких ошибок 3 и более, дружелюбно поругай ученика за незнание синтаксиса, но укажи только на две ошибки. Предложи остальные ошибки найти ученику самостоятельно.\n` +
    `- НЕ НАДО предлагать решения, использующие блоки try-catch или функции. Исходи из стиля программирования ученика.`;
...
async function callOpenAIForAnswerVerdict({ task, code }) {
  ...
  const userPrompt =
    ...
    `- Убедись, что программа написана на Python 3. Если нет, не принимай такой ответ!\n` +
